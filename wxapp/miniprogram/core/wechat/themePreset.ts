 ...
 * 移植自 Android 版 ThemePreset.kt。
 *
 * 设计要点：
 * 1. 每套主题以「标准 CSS 文本」保存，由 wechatHtml 解析后**内联**到每个 HTML 元素，
 ...
 * 3. 兼容性已内置：段落 text-align:justify（两端对齐）、表格 width:100%、代码块
 *    overflow-x:auto（横向滚动）、禁止外链字体/外部 CSS。
 */

export interface WechatTheme {
  key: string; // 与存储 Key 完全一致
  name: string; // 下拉框展示名
  css: string; // 官方默认 CSS
}
...
// ────────────────────────────────────────────────
// 工具：复刻 Kotlin String.trimIndent()
// ────────────────────────────────────────────────
function trimIndent(raw: string): string {
  ...
  let minCommonIndent = Infinity;
  for (const line of lines) {
    if (line.trim() !== '') {
      const w = indentWidth(line);
      if (w < minCommonIndent) minCommonIndent = w;
    }
  }
  if (minCommonIndent === Infinity) minCommonIndent = 0;
  return lines
    .map((line) => {
      if (line.trim() === '') return line.replace(/\s+$/, '');
      return line.substring(minCommonIndent);
    })
    .join('\n');
}
...
const LAW_CLEAN_CSS = trimIndent(`
        body {
          font-family: -apple-system, "PingFang SC", "Microsoft YaHei", sans-serif;
          font-size: 16px;
          line-height: 1.6;
...
// 注意：THEMES 必须置于各 CSS 变量之后，否则会引用尚未初始化的变量。
export const THEMES: WechatTheme[] = [
  { key: 'css_law_blue', name: '法律深蓝普法', css: LAW_BLUE_CSS },
  { key: 'css_law_clean', name: '法律极简纯白', css: LAW_CLEAN_CSS },
  { key: 'css_simple', name: '简约阅读', css: SIMPLE_CSS },
  { key: 'css_tech', name: '科技干货', css: TECH_CSS },
  { key: 'css_business', name: '商务正式', css: BUSINESS_CSS }
];
